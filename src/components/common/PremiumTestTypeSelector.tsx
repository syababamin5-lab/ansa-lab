...
export const PremiumTestTypeSelector: React.FC<PremiumTestTypeSelectorProps> = ({
  ...
  return (
    <div ref={containerRef} className="relative w-full text-xs font-sans">
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 top-full mt-1.5 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden max-h-[380px] flex flex-col animate-in fade-in duration-150">
          ...
          <div className="p-2.5 bg-slate-50 border-b border-slate-100 shrink-0">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-8 py-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-600 font-medium"
                autoFocus
              />
              {searchQuery && (
                <button
              ...
              groupedCategories.map(category => {
                ...
                return (
                  <div key={category} className="space-y-1">
                    ...
                    <div className="px-2.5 py-1 rounded-lg bg-slate-100/90 text-[10.5px] font-black uppercase tracking-wider text-slate-700 font-mono flex items-center gap-1.5 border border-slate-200/80">
                      <Box className="w-3 h-3 text-blue-600 shrink-0" />
                      <span>{category}</span>
                    </div>
                      ...
                      {items.map(option => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-2 rounded-xl transition cursor-pointer flex items-center justify-between gap-2.5 ${
                              isSelected
                          ...
                          >
                            <div className="flex items-center gap-2.5 min-w-0">
                              <span className={`px-2 py-0.5 rounded-lg text-[10px] font-mono font-black shrink-0 ${badgeProps.bg} ${badgeProps.text}`}>
                                {option.code}
                              </span>
                              <div className="min-w-0">
                                <div className="font-extrabold text-slate-900 truncate leading-tight">
                                  {option.name}
                                </div>
                                {option.standard && (
          ...
          {/* FOOTER TOTAL COUNT */}
          <div className="p-2 bg-slate-50 border-t border-slate-100 text-[10px] font-mono font-bold text-slate-500 flex items-center justify-between shrink-0">
            <span>TIMES® ANSA LIMS Test Catalogue</span>
            <span className="text-blue-700">{filteredOptions.length} Jenis Uji Tersedia</span>
          </div>
        </div>
      )}
    </div>
